...
public class UpdateAggregatesTask extends AsyncTask<UpdateAggregatesTask.Param, Void, ArrayList<CensusResponse>> {
	...
	public class Param {
		...
		public Param(Activity a, double latitude, double longitude, TwitchDatabase db) {
			this.a = a;
	...
	protected ArrayList<CensusResponse> doInBackground(Param... param) {
        ...
        params.add(new BasicNameValuePair("deviceHash", TwitchUtils.getDeviceID(p.a)));
        ...
        if(!TwitchUtils.hasSentEmail(p.a)) {
        	String email = TwitchUtils.getEmailAddress(p.a);
        	if(email != null) {
        		params.add(new BasicNameValuePair("userEmail", email));
        	}
        	TwitchUtils.setEmailStatus(p.a, true);
        }
        ...
        ArrayList<CensusResponse> responses = p.db.getCachedResponses(p.a);
        p.db.close();
        
        // Launch CogStudyPostTask if necessary (check flag)
        if(TwitchUtils.shouldUploadDB(p.a)) {
        	new CogStudyPostTask().execute(p.a);
        }
        
        // Launch PhotoRankingDumpTask if necessary
        if(TwitchUtils.shouldDumpPhotoRanking(p.a)) {
        	new PhotoRankingDumpTask().execute(p.a);
        }
