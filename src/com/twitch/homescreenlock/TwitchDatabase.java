...
public class TwitchDatabase {
	...
	/*
	//Method to update the choice of Structure The Web. 
	public void updateStructureTheWeb(int index, String selection, String userSelection, long duration) {
		int rowNumber = index+1;

		ContentValues cvUpdateUserInput = new ContentValues(); 
		cvUpdateUserInput.put(KEY_USERINPUT, userSelection);		
		databaseTwitch.update(DATABASE_TABLE_WEB, cvUpdateUserInput, KEY_ROWID_WEB + "=" + rowNumber, null);	

		ContentValues cvUpdateSelected = new ContentValues(); 
		cvUpdateSelected.put(KEY_WEBSELECTED, selection);		
		databaseTwitch.update(DATABASE_TABLE_WEB, cvUpdateSelected, KEY_ROWID_WEB + "=" + rowNumber, null);	

		ContentValues cvUpdateDuration = new ContentValues(); 
		cvUpdateDuration.put(KEY_WEB_DURATION, duration);		
		databaseTwitch.update(DATABASE_TABLE_WEB, cvUpdateDuration, KEY_ROWID_WEB + "=" + rowNumber, null);	

	}
	
	//Method to update the flag from Selected to NotSelected. For all, to loop the table.  
	public void updateStructureTheWebNotSelectedFlag() {
		ContentValues cvUpdate = new ContentValues(); 
		cvUpdate.put(KEY_WEBSELECTED, "NotSelected"); 
		databaseTwitch.update(DATABASE_TABLE_WEB, cvUpdate, null, null);			
	}
	
	//Methods to get data or information from tables. 
	public String gettabseperatedSentence(int row) {
		Cursor c = databaseTwitch.query( DATABASE_TABLE_WEB, null, KEY_ROWID_WEB + "=" + row, null, null, null, null); 
		try
		{
			if(c!=null && c.moveToFirst())
			{
				c.moveToFirst(); 
				String url = c.getString(1); //1 is for SENTENCE.
				return url; 
			}
		}
		catch(Exception e)
