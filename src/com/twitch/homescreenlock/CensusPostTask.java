...
public class CensusPostTask extends AsyncTask<CensusPostTask.Param, Void, Void> {
	...
	public class Param {
		...
		public Param(Context c, CensusResponse response) {
			this.c = c;
			this.response = response;
		}
	}
	
	public void makeLocalEntry(Context c, CensusResponse response) {
		Log.d("TwitchServer", "Couldn't reach TwitchServer, rerouting response to local db");
		TwitchDatabase db = new TwitchDatabase(c);
    	db.open();
	...
	protected Void doInBackground(Param... param) {
		...
		if(!TwitchUtils.twitchServerOnline()) {
			if(p.c != null) { // if null, can't store locally (happens if lose connection during update)
		    	makeLocalEntry(p.c, p.response);
			}
	    	return null;
		}
