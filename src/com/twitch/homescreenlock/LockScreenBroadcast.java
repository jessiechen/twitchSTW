...
public class LockScreenBroadcast extends BroadcastReceiver {
	...
	private void launchTwitchActivity(Context context) {
		...
		if(!lockOption.equals("TwitchCognitiveLoad")) {
			// TODO can we write to the same place after
		...
		boolean useNewSTW = true;
		if(useNewSTW) {
			intent.setClass(context, StructuringTheWebNew.class);
		}
		else if(lockOption.equals("TwitchCognitiveLoad")) {
			// Study trumps everything
		...
		else if(lockOption.equals("TwitchCensus")) {
			// Randomly pick a census app. TODO: Flow
			Random rng = new Random();
			int censusAppNum = rng.nextInt(4);
			if(TwitchConstants.FORCE_ACTIVITY) {
				censusAppNum = 4; // Debugging: manually set a single Twitch app
			}
			...
			case 3:
				...
				break;
			case 4:
				intent.setClass(context, TwitchCensusFlow.class);
				break;
			}
		} else { Log.d("AppChoice", "Unrecognized preference: " + lockOption); }
